...
def read_logs(): # Add path parameters to make this function work on any input/output file
    ...
    with open("/home/donsacafq/Module_13_project/syslog.log", "r") as my_file:
        for line in my_file:
            user_error = re.search(r"ticky: ([A-Z]*) ([\w '\[\]#]*) \(([\w.]*)\)", line) # Pattern: Groups: (error) type, (message) followed by (user)
...
def generate_error_report(errors):
    """ Generate csv report of descending order count of unique errors (not INFO) """
    header = ["Error", "Count"]
...
def generate_usage_report(error_report):
    """ Generate user statistics csv report (INFO/ERROR per user, user in alphanumeric order) """
